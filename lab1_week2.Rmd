...
ggplot(data = sfo_feb_flights,aes(x = arr_delay)) +
  geom_histogram()
sfo_feb_flights %>%
  summarise(mean_sfo = mean(arr_delay),median_sfo = median(arr_delay), sd_sfo = sd(arr_delay),var_sfo = var(arr_delay),IQR_sfo = IQR(arr_delay),min_sfo = min(arr_delay), max_sfo = max(arr_delay))
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_adc = median(arr_delay), IQR_adc = IQR(arr_delay))
```
...
nycflights %>%
  group_by(tailnum) %>%
  summarise(mean_fast = mean(avg_speed)) %>%
  arrange(desc(mean_fast))
```
...
nycflights %>%
  summarise(on_time_arr = sum(arr_type == "on time")/n()) %>%
  arrange(desc(on_time_arr))
